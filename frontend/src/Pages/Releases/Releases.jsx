...
const Releases = () => {
  ...
  return (
    <div className="min-h-screen overflow-x-hidden">
      ...
      {heroMovie && (
        <div className="relative min-h-[70vh] md:min-h-[80vh] overflow-hidden">
          ...
          <div className="relative h-full min-h-[70vh] md:min-h-[80vh] flex items-center">
            <div className="max-w-[85%] mx-auto px-4 w-full">
              <div className="max-w-3xl animate-fade-up">
                <div className="flex items-center gap-3 mb-4">
                  <span className="rounded-full bg-red-600/20 border border-red-500/30 px-3 py-1 text-xs font-medium text-red-400 backdrop-blur-sm">
                    Featured Release
                  </span>
                  <span className="flex items-center gap-1.5 rounded-full bg-yellow-500/10 border border-yellow-500/20 px-3 py-1 text-xs font-medium text-yellow-400 backdrop-blur-sm">
                  ...
                  {heroMovie.genre_ids && (
                    <span className="flex flex-wrap gap-2">
                      {heroMovie.genre_ids.slice(0, 3).map((id) => (
                        <span key={id} className="rounded-full bg-white/5 px-2.5 py-0.5 text-xs text-gray-300 ring-1 ring-white/10">
                ...
                {heroMovie.overview && (
                  <p className="mt-4 text-sm md:text-base text-gray-400 leading-relaxed max-w-2xl line-clamp-3">
                    {heroMovie.overview}
                  </p>
                )}
                ...
                <div className="mt-6 flex flex-col sm:flex-row gap-3">
                  <button
                    ...
                    className="inline-flex items-center justify-center gap-2 rounded-full bg-red-600 px-6 py-3 text-sm font-semibold text-white shadow-lg shadow-red-500/20 transition-all duration-300 hover:bg-red-500 hover:shadow-xl hover:shadow-red-500/30 active:scale-[0.97]"
                  >
                  ...
                  <Link
                    to={`/movies`}
                    className="inline-flex items-center justify-center gap-2 rounded-full border border-white/10 bg-white/5 px-6 py-3 text-sm font-medium text-gray-300 backdrop-blur-sm transition-all duration-300 hover:border-red-500/30 hover:bg-red-500/10 hover:text-white"
                  >
                    Book Tickets
      ...
      {/* ───── Sections Container ───── */}
      <div className="max-w-[85%] mx-auto px-4 pb-20 -mt-16 md:-mt-20 relative z-10 space-y-14">
        ...
        <section className="animate-fade-up">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-xl md:text-2xl font-bold text-white">Upcoming This Week</h2>
            </div>
          ...
          <div ref={scrollRef} className="flex gap-4 overflow-x-auto pb-4 snap-x snap-mandatory scrollbar-hide">
            {upcoming.map((movie) => (
              <div
                key={movie.id}
                className="min-w-[180px] sm:min-w-[200px] snap-start rounded-xl border border-white/[0.06] bg-[linear-gradient(180deg,rgba(255,255,255,0.04),rgba(255,255,255,0.01))] overflow-hidden transition-all duration-300 hover:border-red-500/20 hover:shadow-[0_0_30px_rgba(239,68,68,0.06)] shrink-0"
              >
                ...
                <div className="p-3">
                  ...
                  {movie.release_date && (
                    <p className="mt-1 text-[11px] text-gray-500">{formatDate(movie.release_date)}</p>
                  )}
                </div>
              </div>
            ))}
        ...
        <section className="animate-fade-up">
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-xl md:text-2xl font-bold text-white">Trending Releases</h2>
            </div>
          </div>
              ...
              >
                <div className="relative aspect-[2/3] overflow-hidden">
                  ...
                  <div className="absolute top-2 left-2 flex flex-col gap-1">
                    {i === 0 && (
                      <span className="rounded-full bg-gradient-to-r from-red-600 to-red-700 px-2.5 py-0.5 text-[10px] font-bold text-white shadow-lg flex items-center gap-1">
                        <Sparkles className="h-3 w-3" />
                        Trending
                      </span>
                    )}
                    {i === 1 && (
                      <span className="rounded-full bg-blue-600/90 px-2.5 py-0.5 text-[10px] font-bold text-white">
                        New
                      </span>
                    )}
                    {movie.vote_average >= 8 && (
                      <span className="rounded-full bg-yellow-500/90 px-2.5 py-0.5 text-[10px] font-bold text-black">
                        Fan Favorite
                      </span>
                    )}
                  </div>
                  ...
                  <div className="mt-1.5 flex items-center gap-2 text-[11px] text-gray-500">
                    ...
                    {movie.genre_ids && movie.genre_ids.length > 0 && (
                      <>
                        ...
                        <span>{movie.genre_ids.slice(0, 2).map((id) => genresList[id - 1] || `Genre ${id}`).join(", ")}</span>
                      </>
                    )}
                  </div>
                </div>
        ...
        {comingSoon.length > 0 && (
          <section className="animate-fade-up">
            <div className="flex items-center gap-3 mb-6">
              ...
              <h2 className="text-xl md:text-2xl font-bold text-white">Coming Soon</h2>
            </div>
                  ...
                  >
                    <div className="relative aspect-[2/3] overflow-hidden">
                      ...
                      <div className="absolute bottom-3 left-3 right-3">
                        <div className="rounded-xl bg-black/70 backdrop-blur-md border border-white/10 px-3 py-2 text-center">
                          <p className="text-[10px] text-gray-400 uppercase tracking-wider">Releasing in</p>
        ...
        <section className="animate-fade-up">
          ...
          {filteredMovies.length > 0 ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 text-center">
              ...
              <p className="text-sm text-gray-500">No movies found for this genre.</p>
              <p className="text-xs text-gray-600 mt-1">Try selecting a different genre.</p>
            </div>
          )}
        </section>
      </div>
      ...
      {trailerModal.open && (
        ...
        >
          ...
          >
            <button
              onClick={() => setTrailerModal({ open: false, key: null })}
              className="absolute -top-3 -right-3 z-10 rounded-full bg-black/80 border border-white/10 p-2 transition-all hover:bg-red-600 hover:scale-110"
            >
